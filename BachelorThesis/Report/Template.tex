...
\begin{document}
\title{Measurement of $\gamma$, \space $\pi^{0}$ and $\eta$ ratios in the photonic background in pp collisions at $\sqrt{s} = 7 TeV$ with ALICE}

\abstract{The heavy flavour production in pp collision is an important testing ground the QCD, the field theory for the strong 
interaction. In this report the main sources of the background for the semileptonic decays is evaluated, for pp collisions at 
$\sqrt{s} = 7$ $TeV$. For this purpose the photonic .}

\shortauthor{Mustermann, Felix}
\shorttitle{Sample file for reports using the gsipaper class}

\author{Felix Mustermann}            % first author
\address{University of Musterstadt, somewhere@nowhere.com}
...
\section{Guidelines}
\subsection{Writing your report}\label{ex:writing}
...
possible. Use the equation environment to display equations that you
would like to reference to
\begin{equation}
 a^2 + b^2 = c^2
\label{ex:eqn:pythagoras}
\end{equation}
like this (see very important Eqn.\ref{ex:eqn:pythagoras}) or use the
{\it eqnarray} environment to split equations into several lines,
and/or to align several equations.


If it's essential to have a two-column wide equation then use the
method of Eqn.(\ref{ex:eq:murnf}).


\begin{table}
\center
\caption{Small Table with some fancy input}\label{ex:tab:smtab}
\begin{tabular}{|c|c|c|c|}
  ...
  \cline{1-2}
  \multicolumn{2}{|c|}{\raisebox{0pt}[12pt][6pt]{and even more importantly}} & &\\\cline{1-2}
...
shown in Tables \ref{ex:tab:smtab} and \ref{ex:tab:exp}. For either a
...
\ref{ex:tab:smtab} and \ref{ex:tab:exp}.  For the inner lines of the
...
Table\ref{ex:tab:exp}. Do \textbf{not} change the latex commands from
{\it $\backslash$begin\{table*\}} to
{\it $\backslash$begin\{tabular\}}, or from
{\it $\backslash$end\{tabular\}} to
{\it $\backslash$end\{table*\}},
apart from inserting your own caption heading and table label. The caption
heading for a table should be placed at the top  of the table.


\begin{figure}[tb]
\center
\includegraphics[scale = 0.3]{QCD_phase_diagram.jpg}
...
The same arguments apply as are given above for tables, i.e. it is
preferable to have figures that fit into one column of the text. If
this is not possible, then use the example of figure and use the
commands {\it $\backslash$begin\{figure*\}} and {\it
  $\backslash$end\{figure*\}}. The {\it $\backslash$includegraphics}
command can take optional arguments as width and length of the
picture, as can be seen in the examples given in this document.
...
As figures use \textit{pdf}- or \textit{jpg}-files. You will find this
format amongst other output formats of most graphics software.  DO NOT
- I repeat - DO NOT use the Unix command \verb|convert| unless you
really know what you are doing!


\begin{table*}[t]
\caption{Nonsense Data. Note the difference between with(out)
  \textit{raisebox} attribute in the code \label{ex:tab:exp}} 
...
and Figures}\label{ex:limits}

In the final stages of preparing the document, try to declare the
two-column wide figures, tables or equations at a point in {\it
  example.tex} that is prior to the top of the column of text where
you would like the item to appear.\footnote{By `declaring' I refer to
  placing the chunk of material describing the table or whatever at a
  particular point in the text file.} You should try to use {\it
...
\subsection{Footnotes, the Bibliography, and Acknowledgments} \label{ex:foot}
...
example, the first paper by Jarlskog\cite{ex:ja} is cited using the
command {\it $\backslash$cite\{ex:ja\}}.
...
This is where one places acknowledgments to your tutors, Albert
Einstein etc.  Note that there are no section numbers for the
Acknowledgments. The style file will automatically generate the
